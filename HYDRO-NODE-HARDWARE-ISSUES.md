...
Version: v28   |   Last updated: 2026-08-31   |   Status: **Stage 1 bring-up.** Sleep-current teardown complete — **7.8 µA measured, against a 25 µA target.** The SF9 battery gamble is off the table

## STATUS SUMMARY
Total issues: 71   |   Open: 44   |   Resolved: 25   |   Won't fix: 2
...
PCB: `Hydro_Node_PCB.PcbDoc`, **80 × 60 mm, rev 2026-08-31** — every net connected, min clearance **1.200 mm**, ground pour on **both** layers, **33 GND stitching vias**, no silkscreen on any pad, all footprints correct. **Cleared for firmware.** Four items before fab (dead copper, mounting-hole size/plating/position, J1 boss). See `PCB-FINAL-CHECK.md`.
...
- Impact on the two-year target, against the 4400 mAh pack and the 1373 mAh of active energy already budgeted:
  ...
  So at **SF7, 100 µA does not fail the two-year target** — it eats the margin, 2.43× down to 1.41×.
- **The real argument, and the one that was under-stated when this issue was first written.** "It still gives two years" is true and was fairly challenged. The answer is not the safety factor in the abstract — it is that **the 1.41× is spent before the link is even measured.** HW-003's margin table is quoted at 25 µA sleep and at +18 dBm on two cells; re-running it at the measured 100 µA:

  | | Active energy | Total at 25 µA sleep | Total at 100 µA sleep |
  |---|---|---|---|
  | **SF7** | 1373 mAh | 1811 mAh → **2.43×** | 3125 mAh → **1.41×** ✅ |
  | **SF9** | 3027 mAh | 3465 mAh → **1.27×** | **4779 mAh → 0.92×** ❌ **FAILS** |
  | SF10 | — | FAILS already | FAILS |

  At SF9 the pack holds 4400 mAh and the design needs 4779 — it runs out roughly two months short. **HW-047's link measurement has not been taken.** 50 m through thick concrete is exactly the case where SF9 is plausible, which is why HW-003 chose two cells rather than one in the first place.
- **So the correct framing is: 100 µA is a bet on the link being easy. 25 µA wins either way, and closing the gap costs five lines of firmware and no parts.** It is not a blocker and it is not urgent, but it should be fixed *before* the link is measured rather than after, so that the link measurement is not simultaneously a battery decision.
- Remaining margin at 25 µA still has to absorb Li-SOCl₂ capacity loss at rooftop temperature, passivation (HW-032) and the diode drops of HW-003.
- **The measurement is not yet valid, for two reasons that must be cleared first:**
  ...
  2. **The sensors were not connected** — confirmed on the bench: Pro Mini and Ra-02 only. So 100 µA is the *board* sleeping, not the *product* sleeping. The finished node adds J2 (~1.5 µA, fine) and **J3**, where the ultrasonic sits on the permanently-powered rail — see **HW-071**. The real product figure is higher than 100 µA by however much that module idles at, and that is not yet measured.
- **What the measurement already proves, and it is worth recording as a pass:**
  - **The Ra-02 is genuinely asleep.** An SX1278 left in standby draws ~1.6 mA. At 100 µA total it cannot be awake, so the firmware's radio-sleep sequence works. That was the largest single risk in the sleep budget and it is closed by this measurement.
  - **The ADC is already disabled.** ADC supply current is 200–300 µA on its own; the whole board is drawing less than half of that.
- **The 100 µA is fully accounted for by four things that are simply switched on and should not be** — BOD ~20 µA, the D13 LED ~40 µA, the floating inputs ~20 µA, the analog comparator ~10 µA, plus the ~5 µA watchdog that is required. That sums to ~95 µA against a measured 100 µA. **There is no unexplained residual**, which means there is no hidden hardware fault to hunt — this is entirely a firmware teardown problem and the fix costs nothing.
...
- **Test firmware written and committed: `firmware/sleep_test/sleep_test.ino`.** One `#define STAGE n` at the top selects a cumulative teardown stage, 0 to 6; seven flashes give the whole attribution. Design notes worth keeping:
  - **No serial output.** A USB-TTL adapter back-feeds the board through RX and DTR and invalidates every reading, so the sketch reports through the D13 LED before it sleeps and the instructions say to pull all six programmer wires before measuring.
  - **The radio is proven asleep, not assumed asleep.** It reads `RegVersion` (0x42) and requires `0x12`, writes `RegOpMode` twice (FSK+sleep, then LoRa+sleep — the LongRangeMode bit only changes while already in sleep), then reads the mode back. 2 fast blinks = confirmed, 6 fast blinks = the reading is meaningless.
  - **SPI is bit-banged**, so no library version can change the result, and so the pin states after the transfer are explicit rather than whatever `SPI.end()` happens to leave behind.
  - **Stages 1–2 deliberately leave D13 as `INPUT_PULLUP`** — the suspected real-firmware state — and stage 3 parks it low. **The stage 2 → 3 delta is a direct measurement of the D13 LED**, which is the whole of HW-046.
  - `ADCSRA = 0` is executed *before* `power_all_disable()`, since taking the ADC's clock away first makes ADEN unreachable. All blinking happens before `power_all_disable()`, since it stops Timer0 and therefore `delay()`.
  - A1, A2, D5 and D4 are explicitly left as plain inputs with a comment saying why: all four already have a defined level from board resistors, and a pull-up on a pin reaching the latch domain is exactly how HW-067 happened.
  - **Verified by compiling all seven stages against real avr-libc for the ATmega328P**, and by disassembling stage 5 to confirm the BOD sequence: `MCUCR |= BODS|BODSE`, `MCUCR = BODS`, `sei` (1 cycle), `sleep` — inside the chip's 3-cycle BODS window. `SMCR` is confirmed set to power-down.
- Method to attribute the current rather than guess at it — the sketch above automates this, but the same order applies to hand-testing:
...
- **Confirmed not yet measured (v26):** the 100 µA reading in HW-070 was taken with **no sensors connected** — Pro Mini and Ra-02 only. So nothing measured so far says anything about this issue, and the product's real sleep current is still unknown.
...
- Severity: MINOR
- Status: OPEN
- Component / net: U1 (Pro Mini) — A0–A7, D0, D1, DTR, and the unconnected Ra-02 DIO lines
...
| Version | Date | Change |
|---|---|---|
| v28 | 2026-08-31 | **New PCB and schematic received and fully re-checked. Cleared for firmware; four small items before fab.** Verdict in `PCB-FINAL-CHECK.md`. Measured improvements: **ground pour now on Top AND Bottom** (was Top only), **33 GND stitching vias** (was 1, and it was on BATT+), min different-net clearance **0.351 → 1.200 mm**, C6 to the radio **12.2 → 5.55 mm**, C8 **8.3 → 3.78 mm**, board **90 × 70 → 80 × 60 mm**, and **no silkscreen within 0.25 mm of any pad** board-wide. All 28 signal nets re-verified fully connected by copper overlap; GND's 22 pads all sit inside both pours. **HW-062 CLOSED** (33 vias, clustered where HW-007 asked — around U3, U1, U2, Q1). **HW-063 CLOSED and its advice withdrawn:** top-layer routing is still 269 mm, but the old recommendation assumed a pour on Top only, so a top trace cut the only return path there was. With pours on both layers plus stitching, moving a trace top→bottom merely moves the slot to the other layer and gains nothing. The two-pour-plus-stitching approach is a **better** fix than the one recommended. **HW-065 half fixed** — silkscreen clear; J1's boss pad still 1.1 mm pad on a 1.1 mm hole, zero ring, needs NPTH. **HW-061 improved but not closed** — C6 at 5.55 mm is well short of the 12.2 mm it was, still short of the <3 mm target; C8 at 3.78 mm is good. **HW-053 re-closed on a better fix than the one signed off in v17:** both `J2.3` **and** the 4.7 kΩ pull-up `R7` have moved to **D3**, so D3 low really does power the probes down — the v17 compromise left R7 on BATT+, which would have held the data line at 3.6 V and kept the probes alive through their own protection diodes. D3 sources ~3.7 mA worst case against a 20 mA pin. **HW-067 permanently fixed in the schematic** — the 100 Ω from A0 to the flip-flop clock is deleted, not just cut on the bench. **HW-069's prototype fix is in the schematic** (R14 = 2.2 MΩ); the Schmitt trigger stays a Rev B item as decided. **NEW finding, flagged before firmware starts: A0 is now permanently disconnected, so the MCU cannot sense the magnet at all.** That is the accepted consequence of the HW-067 fix, but it silently removes half of HW-021 and all of HW-022's magnet-hold gesture — the node can still switch *itself* off via A1, it just cannot *read* the reed. The firmware spec must not assume A0 reads anything; it returns in Rev B on the Schmitt's driven output. **NEW: mounting holes are 2.70 mm** — an M3 screw is 3.0 mm and will not pass; 2.70 mm is the M2.5 clearance size. Either the screw or the hole is wrong, and all four are also plated pads with pad size = hole size, i.e. **zero annular ring**, so they should be NPTH. **HW-066 unchanged and never actioned** — still 2.5 mm from the edge, 1.15 mm of material. **Tooling defect found and fixed: `tools/inspect_pcb.py` reported ZERO pads on this file.** `Pads6` is not framed like the other binary streams — each record is a Pascal name block, three short sub-blocks, a 194-byte body and a zero-length terminator — so the generic `[u8 type][u32 len][payload]` walker desynchronised on the first record. That would have silently skipped **every** footprint, clearance and connectivity check in this review. Bodies are now found by scanning the `\xc2\x00\x00\x00` marker and validating the layer byte, recovering all 141 pads. **Second tooling defect, caught before it became a false blocker:** the parser ignored **pad rotation**, which made Q1's three lead pads appear to overlap by 0.06 mm — a dead short between Gate, Drain and Source. Q1's pads are rotated 270°, so the 1.70 mm dimension runs along the 2.54 mm lead pitch and the real gap is **0.84 mm**. Rotation lives at byte 52 of the pad body as an 8-byte double and is now read. Q1's tab is correctly on GND (the TO-220 tab is internally the Drain, and Drain is the switched ground). Blockers unchanged at 1: **HW-003**, which is a battery-pack assembly item and not on the board. |
